		...
		<nav class="flex-1 space-y-2 px-4 py-6">
			{#each navigation as item (item.href)}
				...
				>
					...
					{#if item.name === 'Alertas' && conteoAlertas > 0}
						<span class="rounded-full px-2 py-0.5 text-xs font-bold {isActive(item.href) ? 'bg-white/30 text-white' : 'bg-red-500 text-white'}">
