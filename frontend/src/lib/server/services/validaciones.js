...
export async function getValidacionesPendientesReentrenamiento(metadata = {}) {
	const validaciones = await sql`
		SELECT
			ev.id_validacion,
			ev.id_evaluacion,
			ev.id_especialista,
			ev.fecha_validacion,
			ev.diagnostico_especialista,
			ev.nivel_confianza,
			ev.util_para_entrenamiento,
			em.id_respuesta,
			pr.respuestas
		FROM evaluacion_validacion ev
		INNER JOIN evaluacion_ml em ON em.id_evaluacion = ev.id_evaluacion
		INNER JOIN paciente_respuesta pr ON pr.id_respuesta = em.id_respuesta
		WHERE ev.util_para_entrenamiento = true
		ORDER BY ev.fecha_validacion ASC, ev.id_validacion ASC
	`;

	const idsIncluidos = new Set((metadata.included_validation_ids ?? []).map((id) => String(id)));
	const snapshotDate = metadata.feedback_snapshot_timestamp
		? new Date(metadata.feedback_snapshot_timestamp)
		: null;

	return validaciones.filter((validacion) => {
		if (!idsIncluidos.has(String(validacion.id_validacion))) {
			return true;
		}
		...
		return new Date(validacion.fecha_validacion) > snapshotDate;
	});
}
